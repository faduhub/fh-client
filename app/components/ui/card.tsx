...
export function Card({
  ...
  const style = useMemo(() => {
    ...
    const p = (shift: number, mod: number, base = 10) =>
      base + ((h >> shift) % mod)
  ...
  return (
    ...
    >
      {showInitial && initial && (
        <span className="absolute inset-0 flex items-center justify-center font-serif text-foreground/90 mix-blend-overlay">
          {initial}
        </span>
      )}
